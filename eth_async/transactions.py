...
class Transactions:
    ...
    async def auto_add_params(self, tx_params: TxParams) -> TxParams:
        ...
        if 'gasPrice' not in tx_params and 'maxFeePerGas' not in tx_params:
            ...
            # else:
            tx_params['gasPrice'] = int(gas_price * 1.5)
    ...
    async def sign_and_send(self, tx_params: TxParams) -> Tx | None:
        """
    ...
    async def approve(
            ...
            gas_limit: types.GasLimit | None = None, nonce: int | None = None, max: bool | None = False,
            station_max: bool | None = False
    ) -> Tx:
        """
        ...
        if max:
            tx_params['data'] = tx_params['data'][:74] + ('f' * 64)

        if station_max:
            tx_params['data'] = tx_params['data'][:74] + '7' + ('f' * 63)
